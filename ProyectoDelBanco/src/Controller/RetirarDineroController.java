...
public class RetirarDineroController {
	 ...
	 */
	private Cliente cliente = MetodosCuenta.banco.getCliente();
    ...
    void btnRetirarDineroEvent(ActionEvent event) {
    	...
    	if(this.txtDineroDepositar.getText().equals("")) {
        	...
        	alert.showAndWait();
    	}else if(verificarDineroNegativo(dineroRetirar)) {
    		
        	Alert alert = new Alert(Alert.AlertType.INFORMATION);
        	alert.setHeaderText(null);
        	alert.setTitle("Notificacion");
        	alert.setContentText("El valor no puede ser menor o igual a 0.");
        	alert.showAndWait();
        	
    	}else {
    		if(verificarDinero(dineroRetirar)) {
    	    	
    	    	Double dineroDelCliente = MetodosCuenta.banco.getCliente().getDefCuenta().getSaldo();

        		Double retirarDinero = MetodosCuenta.banco.getCliente().getDefCuenta().setSaldo(dineroDelCliente - dineroRetirar);
    ...
    private boolean verificarDineroNegativo(double dineroRetirar) {
    	if(dineroRetirar <= 0) {
    		return true;
    	}
    	return false;
    ...
    private void crearTransaccion(double dineroRetirar) {
    	...
    	MetodosCuenta.banco.crearTransaccion(transaccionNueva);
  ...
  //__________________________________________________________________
	...
	private boolean verificarDinero(double dineroRetirar) {
    	...
    	if(dineroRetirar <= MetodosCuenta.banco.getCliente().getDefCuenta().getSaldo()) {
    		return true;
    	}
		return false;
	}
