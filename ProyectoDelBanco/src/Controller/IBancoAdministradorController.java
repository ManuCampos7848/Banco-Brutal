...
public class IBancoAdministradorController implements Initializable{
	...
	public void initialize(URL location, ResourceBundle resources) {
		 ...
		 */
	    Iterator<Empleado> it = MetodosBanco.banco.getListaEmpleados().iterator();
	    while (it.hasNext()) {
	...
	private void cargarClientes() {
		 ...
		 */
		Iterator<Cliente> iterador = MetodosBanco.banco.getListaClientes().iterator();
    ...
    void agregarEvent(ActionEvent event) {
    	...
    	if(this.txtNombre.getText().equals("") || this.txtApellidos.getText().equals("") || this.txtCedulaCliente.getText().equals("") ||
    		...
    		if(MetodosBanco.banco.verificarCedulaCliente(nuevoCliente)) {
    			
    			if(!this.clientes.contains(nuevoCliente)){
    				this.clientes.add(nuevoCliente);
    				this.tblDatos1.setItems(clientes);

    				MetodosBanco.crearCliente(nuevoCliente);
    	...
    	if(cliente == null){
    		...
    		try{
    			...
    			if(verificacionCamposCliente(nuevoCliente)) {
        			...
        			alert.setHeaderText(null);
        			alert.setTitle("Error");
        			alert.setContentText("Por favor, complete todos los campos, en caso de olvidar uno, escriba su atributo faltante o de click nuevamente en seleccionar cliente.");
        			alert.showAndWait();
        			...
        			MetodosBanco.actualizarCliente(nuevoCliente);
    ...
    void buscarEvent(ActionEvent event) {
    	...
    	if(cedula.equals("")) {
    		...
    		alert.setHeaderText(null);
    		alert.setTitle("Error");
    		alert.setContentText("Primero digite la cedula del cliente.");
    		alert.showAndWait();
    	}else {
    		String busquedadCliente = MetodosBanco.obtenerCliente(cedula);

    		Cliente objeto = MetodosBanco.obtenerObjetoCliente(cedula);

    		if(busquedadCliente == null || objeto == null) {
    			textArea.setText("El cliente no existe \nen la base de datos.");
    		}else {
    			textArea.setText(busquedadCliente.toString() + "\n" + "Numero de cuenta: " + objeto.getDefCuenta().getNumeroCuenta() + "\n"
    					+ "Saldo Actual: " + objeto.getDefCuenta().getSaldo() + "\n" + "Tipo de Cuenta: " + objeto.getCuenta());
    		}
    	}
    ...
    void eliminarEvent(ActionEvent event) {
    	...
    	if(clienteSeleccionado == null){
    		...
    		boolean eliminacionCliente  = MetodosBanco.eliminarCliente(clienteSeleccionado);
    ...
    void selecEvent(ActionEvent event) {
		...
		if(cliente == null) {
			...
			alert.setHeaderText(null);
			alert.setTitle("Error");
			alert.setContentText("Primero seleccione un cliente.");
			alert.showAndWait();
		}else {
			...
			alert.setHeaderText(null);
			alert.setTitle("Info");
			alert.setContentText("Modifique unicamente los necesario, por favor no borrar el resto de información de los campos.");
			alert.showAndWait();
		}
    ...
    void hacerDepositoEvent(ActionEvent event) {
    	...
    	Cliente clienteSeleccionado = this.tblDatos1.getSelectionModel().getSelectedItem();
    	MetodosBanco.banco.setCliente(clienteSeleccionado);
    ...
    void consultarSaldoEvent(ActionEvent event) {
    	...
    	if(clienteSeleccionado == null) {
    		...
    		MetodosBanco.consultarSaldo(clienteSeleccionado);
    ...
    void agregarEvent2(ActionEvent event) {
    	...
    	if(this.txtNombre2.getText().equals("") || this.txtApellidos2.getText().equals("") || this.txtCedulaCliente2.getText().equals("") ||
    		...
    		if(MetodosBanco.banco.verificarCedulaEmpleado(nuevoEmpleado)) {
    			
    			if(!this.empleados.contains(nuevoEmpleado)){
    				this.empleados.add(nuevoEmpleado);
    				this.tblDatos2.setItems(empleados);
    				
    				MetodosBanco.crearEmpleado(nuevoEmpleado);
    ...
    void buscarEvent2(ActionEvent event) {
    	...
    	if(cedula.equals("")) {
			...
			alert.setHeaderText(null);
			alert.setTitle("Error");
			alert.setContentText("Primero digite la cedula de un empleado");
			alert.showAndWait();
    	}else {
    		String busquedadEmpleado= MetodosBanco.obtenerEmpleado(cedula);

        	Empleado objeto = MetodosBanco.obtenerObjetoEmpleado(cedula);

        	if(busquedadEmpleado == null || objeto == null) {
        		textArea2.setText("El empleado no existe \nen la base de datos.");
        	}else {
    ...
    void eliminarEvent2(ActionEvent event) {
    	...
    	if(empleadoSeleccionado == null){
    		...
    		boolean eliminacionCliente = MetodosBanco.eliminarEmpleado(empleadoSeleccionado);
    	...
    	if(empleado == null){
    		...
    		try{
    			...
    			if(verificarCamposEmpleado(nuevoEmpleado)) {
    				...
    				alert.setHeaderText(null);
    				alert.setTitle("Error");
        			alert.setContentText("Por favor, complete todos los campos, en caso de olvidar uno, escriba su atributo faltante o de click nuevamente en seleccionar empleado.");
    				alert.showAndWait();
    			}else {
    				...
    				MetodosBanco.actualizarEmpleado(nuevoEmpleado);
    ...
    void selecEvent2(ActionEvent event) {
    	...
    	if(empleado == null) {
    		...
    		alert.setHeaderText(null);
    		alert.setTitle("Error");
    		alert.setContentText("Primero seleccione el empleado");
    		alert.showAndWait();
    	}else {
